...
import { useCreateAssessmentRankingSheet } from '@/hooks/use-create-assessment-ranking-sheet'
import { useGetAssessmentRanking } from '@/hooks/use-get-assessment-ranking'
...
export function OverallWithoutBehavior() {
  ...
  const { ranking, students, isLoading } = useGetAssessmentRanking({
    courseId: String(courseId),
    page,
  })

  const { mutateAsync: createAssessmentRankingSheetFn } =
    useCreateAssessmentRankingSheet()

  async function handleDownloadExcel() {
    try {
      const response = await createAssessmentRankingSheetFn({
        courseId: String(courseId),
      })
