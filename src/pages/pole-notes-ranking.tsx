...
import { useGetPoleAssessmentRanking } from '@/hooks/use-get-pole-assessment-ranking'
import { conceptMap, overallStatusMap } from '@/utils/status-and-concept-mapper'

export function PoleNotesRanking() {
  const { id } = useParams()
  const [searchParams] = useSearchParams()
  const courseId = searchParams.get('courseId')
  const page = searchParams.get('page') ?? '1'

  const { ranking, isLoading, error } = useGetPoleAssessmentRanking({
